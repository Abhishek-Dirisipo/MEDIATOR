...
> [!CAUTION]
> ## FOR AUTHORIZED RED TEAMING & SECURITY RESEARCH ONLY
>
...
|:---|:---|
| **Hardware Keylogger** | Silently captures every keystroke - letters, modifiers, special keys, CAPS LOCK state - logged to flash memory that **survives reboots and power cuts** |
| **Remote Key Injector** | Type any text into the target PC remotely via a browser - with speed control and human-like jitter to defeat typing pattern detection |
| **Identity Spoofing** | Reads the real keyboard's USB VID, PID, Manufacturer and Product string, then re-enumerates to the PC as **that exact keyboard** - the Pico is invisible |
...
| **Zero-Lag Bridging** | Keystroke forwarding runs on a dedicated core (Core 1) so capture and Wi-Fi overhead never introduce latency to the user |
...
1. **Plug in** - MEDIATOR sits between the keyboard and PC. The PC re-enumerates it as the exact same keyboard via spoofed identity.
2. **Capture** - Every HID report is intercepted, decoded, and logged to flash. CAPS LOCK state, modifiers, and all special keys are captured with correct case tracking.
3. **Exfiltrate** - Connect to the Pico's Wi-Fi from any device and navigate to `http://<pico-ip>` to view the live keystroke log.
...
All special keys are stored as readable tags in the log. CAPS LOCK state is tracked natively on hardware so uppercase and lowercase are always correct in the raw log - no post-processing needed.
...
```
  ...
  +---[ Persistent Zone 256KB ]---+  0x37F000             (50% split - NEVER auto-erased)
  +---[ Circular Buffer 256KB ]---+  0x3BF000             (50% split - overwrites oldest data)
```
...
- **Clear controls** - Erase flash independently per zone
- **Injection panel** - Type text remotely with speed control
